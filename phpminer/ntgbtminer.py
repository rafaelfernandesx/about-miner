...
def block_mine(block_template, coinbase_message, extranonce_start, address, debugnonce_start=False, timeout=None):
    """
    Mine a block.
        ...
        while nonce <= 0xffffffff:
            ...
            #     hash_rate_count += 1
                ...
                if timeout and (time_stamp - time_start) > timeout:
                    return (None, 123, nonce)

            nonce += 1
        extranonce += 1

    # If we ran out of extra nonces, return none
    return (None, hash_rate, nonce)
...
def standalone_miner(coinbase_message, address):
    ...
    while True:
        ...
        mined_block, hash_rate, current_nonce = block_mine(block_template, coinbase_message, 0, address, current_nonce, timeout=5)
        print("    {:.4f} KH/s".format(hash_rate / 1000.0), "curNonce: " + str(current_nonce))
...
if __name__ == "__main__":
    ...
    #     sys.exit(1)
    msg = 'Hello from vsergeev'
    wallet = '1QB79A754AaKfXwynBEJe6NTrnGrdWHUNF'

    standalone_miner(msg.encode().hex(), wallet)
