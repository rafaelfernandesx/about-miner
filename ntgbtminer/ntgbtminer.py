...
RPC_USER = os.environ.get("RPC_USER", "rafael")
RPC_PASS = os.environ.get("RPC_PASS", "961961961")
...
def rpc_submitblock(block_submission):
    rpc("submitblock", [block_submission])
    print(block_submission)
...
def int2lehex(value, width):
    ...
    """
    value = value.to_bytes(width, byteorder='little').hex()
    # print(value, width)
    return value
...
def int2varinthex(value):
    ...
    """
    if value < 0xfd:
        fmtvalue = int2lehex(value, 1)
    elif value <= 0xffff:
        fmtvalue = "fd" + int2lehex(value, 2)
    elif value <= 0xffffffff:
        fmtvalue = "fe" + int2lehex(value, 4)
    else:
        fmtvalue = "ff" + int2lehex(value, 8)
...
def tx_encode_coinbase_height(height):
    ...
    res = bytes([width]).hex() + int2lehex(height, width)
    return res
...
def tx_compute_hash(tx):
    ...
    tx_hash = hashlib.sha256(hashlib.sha256(bytes.fromhex(tx)).digest()).digest()[::-1].hex()
    return tx_hash
...
def standalone_miner(coinbase_message, address):
    while True:
        ...
        mined_block, hash_rate = block_mine(block_template, coinbase_message, 271161703, address, 5)
